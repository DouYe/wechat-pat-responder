...
- 随机行为：抽象短句、多行发癫文学、认真人生金句、计数器、连击、假系统文字、红包兑奖彩蛋。
- 内置更疯的回复库；自定义回复不再限制长度。
- 随机回复库占 90%，计数器 4%，连击 3%，假系统消息 2%，红包彩蛋 1%。
- 另外同步了
  [Google Doc 词条源](https://docs.google.com/document/d/1zaxLelnWjSDkGEm1SFQnPh643NF7KGJXdcfjVFv7QdM/edit?tab=t.0)
  中的 25 个行为：一级编号是一次随机行为，二级条目会按顺序作为额外的独立消息发送。
...
回复编辑框用独占一行的 `---` 分隔两条随机回复。除此之外的普通换行和空白段落
都会原样保留，例如：
...
编辑框中独占一行的 `>>>` 表示“同一个随机行为里的下一条独立消息”。它不同于
普通换行：程序会按顺序分别粘贴并点击发送，中间短暂停顿。例如：
...
Google Doc 目前不是匿名公开文档，因此由维护者把指定 revision 同步进源代码，
程序运行时不会匿名联网读取。
